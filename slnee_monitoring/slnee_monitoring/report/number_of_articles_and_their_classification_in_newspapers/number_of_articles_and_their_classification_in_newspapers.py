...
def get_item_price_qty_data(filters):
    ...
    item_results = frappe.db.sql("""
				select distinct
						...
						(select count(name) from `tabPress Monitoring` w where w.content_overall_rating ='Positive' and a.Journal_name = w.Journal_name {conditions} ) as number_of_positive_press,
						(select count(name) from `tabPress Monitoring` z where z.content_overall_rating ='Neutral' and a.Journal_name = z.Journal_name {conditions} ) as number_of_neutral_press,
						(select count(name) from `tabPress Monitoring` x where x.content_overall_rating ='Negative' and a.Journal_name = x.Journal_name {conditions} ) as number_of_negative_press,
  						(select count(name) from `tabPress Monitoring` b where a.journal_name = b.journal_name {conditions} ) as count_of_press_release
