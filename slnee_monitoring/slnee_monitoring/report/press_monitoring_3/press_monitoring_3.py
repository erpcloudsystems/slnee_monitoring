...
def get_item_price_qty_data(filters):
    ...
    item_results = frappe.db.sql("""
				select distinct
						a.agency as agency,
						(select count(name) from `tabPress Monitoring` b where b.service_classification ='خبر' and a.agency = b.agency and a.service_classification = b.service_classification {conditions} ) as count_of_press_release,
						(select count(name) from `tabPress Monitoring` c where c.content_item ='مقالات' and a.journal = c.journal and a.service_classification = c.service_classification {conditions} ) as articles,
						(select count(name) from `tabPress Monitoring` f where f.content_item ='تحقيق' and a.journal = f.journal and a.service_classification = f.service_classification {conditions} ) as count_of_press_report,
						(select count(name) from `tabPress Monitoring` e where e.content_overall_rating ='Negative' and e.content_item in ('خبر','مقالات','تحقيق') and a.journal = e.journal and  a.service_classification = e.service_classification {conditions} ) as number_of_negative_press,
						(select count(name) from `tabPress Monitoring` s where s.content_overall_rating ='Positive' and s.content_item in ('خبر','مقالات','تحقيق') and a.journal = s.journal and a.service_classification = s.service_classification {conditions} ) as number_of_positive_press,
						(select count(name) from `tabPress Monitoring` w where w.content_overall_rating ='Neutral' and w.content_item in ('خبر','مقالات','تحقيق') and a.journal = w.journal and a.service_classification = w.service_classification {conditions} ) as number_of_neutral_press
				from
