...
def get_item_price_qty_data(filters):
    ...
    item_results = frappe.db.sql("""
				select distinct
						...
						(select count(name) from `tabPress Monitoring` w where w.content_overall_rating ='Neutral' and a.Journal_name = w.Journal_name {conditions} ) as number_of_neutral_press
				from
				`tabPress Monitoring` a

				where
				(select count(name) from `tabPress Monitoring` w where w.content_overall_rating ='Neutral' and a.Journal_name = w.Journal_name {conditions} ) > 0
				and
				a.docstatus != 2
				{conditions}
