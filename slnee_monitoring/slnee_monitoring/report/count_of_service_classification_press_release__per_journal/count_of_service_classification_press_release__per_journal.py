...
def get_item_price_qty_data(filters):
    ...
    if filters.get("service_classification"):
        conditions += " and a.service_classification =%(service_classification)s"
    item_results = frappe.db.sql("""
				select distinct
						a.journal_name as journal_name,
  						(select count(name) from `tabPress Monitoring` b where a.journal_name =b.journal_name and a.service_classification = b.service_classification {conditions} ) as count_of_press_monitiring
